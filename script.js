...
const renderSinglePartyById = async (id) => {
  ...
  // *** Commented out bc dev said it was buggy ****
    ...
    partyDetailsElement.innerHTML =/* html*/ `
            <h2>${party.name}</h2>
    ...
    closeButton.addEventListener('click', () => {
      ...
      init();
      // will restart the function after clicking the close button fetching all the data again
    });
  } catch (error) {
    console.error(error);
  }
  ...
  try {
    ...
    parties.forEach((party) => {
      ...
      detailsButton.addEventListener('click', async (event) => {
        ...
        console.log("I will show the details");
        partyContainer.innerHTML = "";
        //should remove all innerhtml of all parties but still getch all party details
      ...
      deleteButton.addEventListener('click', async (event) => {
        event.preventDefault();
        console.log("I will delete this shit");
        partyContainer.innerHTML = "";
        renderSinglePartyById(party.id)
      });
    });
